...
│   ├── start_otfz_ignore_printer.py
│   ├── start_otfz_ignore_printer_error.bat
│   ├── RUN_OTFZ_NOW.bat
│   ├── RUN_OTFZ_NOW_SILENT.bat
│   ├── RUN_OTFZ_DIAGNOSTIC.bat
│   └── RUN_OTFZ_DIAGNOSTIC_NO_PRECHECK.bat
└── README.md
```
...
14. `09-DEV/start_otfz_ignore_printer.py`
15. `09-DEV/start_otfz_ignore_printer_error.bat`
16. `09-DEV/RUN_OTFZ_NOW.bat`
17. `09-DEV/RUN_OTFZ_NOW_SILENT.bat`
18. `09-DEV/RUN_OTFZ_DIAGNOSTIC.bat`
19. `09-DEV/RUN_OTFZ_DIAGNOSTIC_NO_PRECHECK.bat`
...
A printer-bypass launcher pair is also included for Windows environments where device initialization fails because no printer is connected. That wrapper sets printer-optional environment flags, installs runtime stubs for common Windows printer modules, and starts FastAPI without requiring a physical printer.

A final ultra-minimal launcher is also included as `RUN_OTFZ_NOW.bat` for situations where you want the shortest possible startup path with printer-ignore environment variables and no extra prompts.

An even quieter variant, `RUN_OTFZ_NOW_SILENT.bat`, starts the same printer-bypass path in a minimized command shell and suppresses stdout and stderr output for users who want the most silent possible local startup.

A diagnostic variant, `RUN_OTFZ_DIAGNOSTIC.bat`, captures precheck results, launcher output, and FastAPI stderr/stdout into a local log file for troubleshooting startup failures on Windows systems.

A second diagnostic variant, `RUN_OTFZ_DIAGNOSTIC_NO_PRECHECK.bat`, skips precheck entirely and writes only launcher and FastAPI output into a local log file. This is intended for cases where the precheck script path itself is failing or Windows cannot open the precheck file.
